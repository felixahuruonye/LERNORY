...
KEY_WORDS: [keywords separated by commas]`,
  ...
  // Generate exam questions with LEARNORY - Feature: Real exam questions generated by AI
  app.post('/api/cbt/generate-questions', isAuthenticated, async (req: any, res: Response) => {
    try {
      const { examType, subject, count = 50 } = req.body;
      ...
      const questions = await generateQuestionsWithLEARNORY(examType, subject, Math.min(count, 50));
      res.json({ questions });
    } catch (error: any) {
